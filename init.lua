...
do
  ...
  -- vim.cmd.colorscheme 'tokyonight-night' -- OVERRIDE
  -- END OVERRIDE
  ...
  map_combo(mode, 'jk', function()
    ...
    if current_mode == 'i' then
      -- 1. Get the current cursor position (0-indexed row, column)
      local cursor = vim.api.nvim_win_get_cursor(0)
      local row = cursor[1] - 1
      local col = cursor[2]

      -- 2. Remove exactly the 2 trailing staging characters ('j' and 'k') directly from buffer text
      -- This modifies the string data directly without executing an "Insert Mode Backspace" event
      vim.api.nvim_buf_set_text(0, row, col - 2, row, col, {})

      -- 3. Move the cursor back 2 spaces to compensate for the deleted characters
      vim.api.nvim_win_set_cursor(0, { cursor[1], math.max(0, col - 2) })

      -- 4. Check if the buffer was clean before typing 'jk'
      -- If it was clean, we force it to remain clean, bypassing the modified marker
      if not vim.bo.modified then vim.bo.modified = false end

      -- 5. Escape cleanly to normal mode
      return '<Esc>'
  ...
  map_combo(mode, 'kj', function()
    ...
    if current_mode == 'i' then
      ...
      -- 2. Remove exactly the 2 trailing staging characters ('k' and 'j') directly from buffer text
      -- This modifies the string data directly without executing an "Insert Mode Backspace" event
      vim.api.nvim_buf_set_text(0, row, col - 2, row, col, {})
      ...
      -- 4. Check if the buffer was clean before typing 'kj'
      -- If it was clean, we force it to remain clean, bypassing the modified marker
      if not vim.bo.modified then vim.bo.modified = false end
...
do
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      local buf = event.buf
      local client = vim.lsp.get_client_by_id(event.data.client_id) -- OVERRIDE
...
do
  ...
  --
  require 'kickstart.plugins.debug' -- OVERRIDE
  require 'kickstart.plugins.indent_line' -- OVERRIDE
  -- require 'kickstart.plugins.lint'
  -- require 'kickstart.plugins.autopairs'
  require 'kickstart.plugins.neo-tree' -- OVERRIDE
